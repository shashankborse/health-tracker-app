...
- **Nutrition logging**: meal-level entries (breakfast/lunch/dinner/snack) against a local food database seeded from Open Food Facts (packaged/barcode items, queried live rather than bulk-imported, since the full dataset is millions of rows), the UK CoFID dataset (generic foods, one-time import from the official 2021 release), and manually entered Irish/UK restaurant-chain items — plus a manual-entry fallback for any scanned/searched item with no match. *Technical note:* the food database also captures sugar, saturated fat, sodium, and a full vitamin/mineral panel (from CoFID's Inorganics/Vitamins sheets and Open Food Facts' equivalent fields, where reported) alongside the core calorie/protein/carb/fat/fibre figures — added mid-build once nutrition logging existed, since a healthy-lifestyle read on a food needs more than just macros. *Acceptance:* logging a meal updates daily calorie/macro/fibre totals immediately, plus sugar/saturated fat/sodium; the fuller vitamin/mineral panel is captured in the database even though no dedicated breakdown view exists yet.
...
- ~~**[Non-blocking — user]** Photo/video poses are assumed as front/side/back by default for the weekly progress entry — confirm or adjust if a different set of angles is preferred.~~ **Resolved**: front, left side, right side, back (four poses).
